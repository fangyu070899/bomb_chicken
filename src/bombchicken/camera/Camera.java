...
import bombchicken.gameobj.GameObject;
...
public class Camera extends GameObject implements KeyListener {
    ...
    private void chaseMove() { //鏡頭追蹤加速度 數字越大追越慢
        ...
        double targeY = (obj.painter().centerY() - painter().centerY()-150) / chaseDivisorY;
    ...
    public void start(Graphics g, boolean pvp) { 
        Graphics2D g2d = (Graphics2D) g;
        tmpCanvas = g2d.getTransform(); // 暫存畫布
...
//            double n = 0.6;
         double n=1;
            g2d.scale(n, n); //做縮放
            // 先將畫布初始移到(0,0)（-painter().left()/top()) 然後再到自己想定位的地點(+ cameraWindowX/Y)
            g2d.translate(-painter().left() + cameraWindowX / n, -painter().top() + cameraWindowY / n);
        } else {
            g2d.translate(-painter().left() + cameraWindowX, -painter().top() + cameraWindowY);
        }
    ...
    public void update() {
        ...
        if (cameraShake) { // 鏡頭搖晃
            shakeCount += 2;
            shakeX = (shakeLevel * Math.sin(shakeCount + Global.random(-1, 1)));
            shakeY = (shakeLevel * Math.cos(shakeCount + Global.random(-1, 1)));
            if (shakeX != 0 || shakeY != 0) {
                translateX((int) shakeX);
                translateY((int) shakeY);
                shakeX = 0;
                shakeY = 0;
            }
//            translateX(Global.random(-1, 1));
//            translateY(Global.random(-1, 1));
            if (shakeDelay.count()) {
                cameraShake = false;
            }
        }
    ...
    public void cameraShake() { // 呼叫此法啟動鏡頭晃動
        cameraShake = true;
        shakeDelay.stop(); // 歸0
        shakeDelay.play(); // 重算
    }
