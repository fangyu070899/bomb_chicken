...
//        arc.stop(new Path().sound().BackGroundMusic().Menu()); // 結束背景音樂
    ...
    public void paint(Graphics g) {
        ...
        if (exitPop.isShow()) {
            ...
            g.drawString("Level: "+level, 490, 290);
...
//                }
            ...
            public void keyReleased(int commandCode, long trigTime) {
                if (exitPop.isShow()) {
                    ...
                    if (commandCode == Global.P1LEFT) {  //彈出視窗
                        level-=1;
                        if(level<=0) {
                        	level=2;
                        }
                    }
                    if (commandCode == Global.P1RIGHT) {  //彈出視窗
                        level+=1;
                        if(level>=3) {
                        	level=1;
                        }
                ...
                if (commandCode == Global.ESC) {
                  ...
                  if (!exitPop.isShow()) { //之後按 esc 開啟離開選項
                        chickenButton.setPosition(440, 280);
                        exitPop.sceneBegin();
                        exitPop.show();
                    } 
