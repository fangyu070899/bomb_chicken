...
public class PvPScene1 extends Scene {
...
//   private int mapHeight = 2170;
   ...
   private int mapWidth = 5500;
   private int mapHeight = 2500;
   private int pX;
   private int pY;
    ...
    public void sceneBegin() {
        ...
        backGrounds.add(new BackGround(0, 0, mapWidth, mapHeight, BackGround.Type.BLACK));
        ...
        mapInitialize(); //初始化地圖
        cArr.add(new Chicken(300, 240, Global.UNIT_X, Global.UNIT_Y));
        cArr.add(new Chicken(400, 240, Global.UNIT_X, Global.UNIT_Y));
//      cArr.add(new Chicken(4858, 1734, Global.UNIT_X, Global.UNIT_Y));
//      cArr.add(new Chicken(4858, 1634, Global.UNIT_X, Global.UNIT_Y));
        ...
        //復活點生成
        rebirthPointArr.add(new Water(2754,350, Global.UNIT_X, Global.UNIT_Y, PopsAnimator.State.REBIRTHPOINTD));
        rebirthPointArr.add(new Water(3210,680, Global.UNIT_X, Global.UNIT_Y, PopsAnimator.State.REBIRTHPOINTD));
        rebirthPointArr.add(new Water(4490,140, Global.UNIT_X, Global.UNIT_Y, PopsAnimator.State.REBIRTHPOINTD));
        rebirthPointArr.add(new Water(4858,1280, Global.UNIT_X, Global.UNIT_Y, PopsAnimator.State.REBIRTHPOINTD));
        ...
        //寶石生成
        gArr.add(new Gem(2323,400,UNIT_X, UNIT_Y, PopsAnimator.State.GEM));
        gArr.add(new Gem(3774,165,UNIT_X, UNIT_Y, PopsAnimator.State.GEM));
        gArr.add(new Gem(4254,355,UNIT_X, UNIT_Y, PopsAnimator.State.GEM));
        gArr.add(new Gem(4321,355,UNIT_X, UNIT_Y, PopsAnimator.State.GEM));
        gArr.add(new Gem(4855,765,UNIT_X, UNIT_Y, PopsAnimator.State.GEM));
...
//        wArr.add(new Worker(616, 472,Global.UNIT_X *3/2, Global.UNIT_Y*3/2,Worker.Type.SAUCE));
      wArr.add(new Worker(2900, 400,64*5/4, Global.UNIT_Y*5/4,Worker.Type.PIG));
      wArr.add(new Worker(3590, 1100,Global.UNIT_X *3/2, Global.UNIT_Y*3/2,Worker.Type.SAUCE));
      wArr.add(new Worker(2850, 860,74*5/4, Global.UNIT_Y*5/4,Worker.Type.PHONE));
      wArr.add(new Worker(3850, 1113,74*5/4, Global.UNIT_Y*5/4,Worker.Type.PHONE));
      wArr.add(new Worker(4260, 225,74*5/4, Global.UNIT_Y*5/4,Worker.Type.PHONE));
      
      //炸雞生成
      fcArr.add(new FriedChicken(1683, 605,63*5/4, Global.UNIT_Y*5/4,FriedChicken.Type.WALK));
        
        eArr.add(new Elevator(5327, 1859, Global.UNIT_X * 4 / 3, 125));
...
//        cameraArr.add(new Camera.Builder(Global.SCREEN_X , Global.SCREEN_Y)
        cameraArr.add(new Camera.Builder(Global.SCREEN_X /2, Global.SCREEN_Y)
                ...
                .setCameraWindowLocation(Global.SCREEN_X /2+10, 0)
                .gen());
        smallMap = new SmallMap(
                new Camera.Builder(mapWidth, mapHeight)
                .setCameraWindowLocation(450, 530)// 小地圖顯示在視窗的(625,440)位置
                .setCameraStartLocation(0, 0)// 鏡頭起始位置設成(0,0)的位置
                .gen(), 0.05, 0.05);// X軸及Y軸縮放比率皆為0.2
...
//        arc.stop(new Path().sound().BackGroundMusic().LavaThemeLayer2()); // 結束背景音樂
    ...
    public void paint(Graphics g) {
        for (int n = 0; n < cameraArr.size(); n++) {
            ...
            for (int i = 0; i < cArr.size(); i++) {
                if (cameraArr.get(n).isCollision(cArr.get(i))&&cArr.get(i).getVisibleState()==false) {
                    cArr.get(i).paint(g);
                }
            }
...
//     seperate line
        ...
        g.fillRect(Global.SCREEN_X / 2, 0, 10, 3000);
        ...
        for (int i = 0; i < bArr.size(); i++) {
            smallMap.paint(g, bArr.get(i), Color.ORANGE);
        }
       
        smallMap.paint(g, cArr.get(0), Color.WHITE);
        smallMap.paint(g, cArr.get(1), Color.PINK);

        for (int i = 0; i < sArr.size(); i++) {
            smallMap.paint(g, sArr.get(i), Color.LIGHT_GRAY);
        }
        for (int i = 0; i < gArr.size(); i++) {
            smallMap.paint(g, gArr.get(i), Color.YELLOW);
        }     
        for (int i = 0; i < eArr.size(); i++) {
            smallMap.paint(g, eArr.get(i), Color.YELLOW);
        }
        for (int i = 0; i < wArr.size(); i++) {
            smallMap.paint(g, wArr.get(i), Color.RED);
        }
        for (int i = 0; i < dArr.size(); i++) {
            smallMap.paint(g, dArr.get(i), Color.BLUE);
        }
        for (int i = 0; i < pressBtnArr.size(); i++) {
            smallMap.paint(g, pressBtnArr.get(i), Color.BLUE);
        }
        for (int i = 0; i < transArr.size(); i++) {
            smallMap.paint(g, transArr.get(i), Color.GREEN);
        }
        for (int i = 0; i < fArr.size(); i++) {
...
//        雞生命和獲得寶石數
        ...
        g.drawString(cArr.get(1).getGemNumber() + "", 140, 76);
        g.drawImage(gemLabel, 90, 50, Global.UNIT_X / 2, Global.UNIT_Y / 2, null);
        ...
        for (int j = 0; j < cArr.size(); j++) {
                 ...
                 for (int i = 0; i < eArr.size(); i++) {
                     if (cArr.get(j).touch(eArr.get(i))) { // 如果雞碰到電梯
                         if (eArr.get(i).getState() == PopsAnimator.State.ELEVATORSTOP) {
                         	PopsAnimator.setElevatorColor(cArr.get(j).getColor());
            ...
            for (int i = 0; i < cArr.size(); i++) {
                if (i != j && !cArr.get(i).getDead()) { // 排除判斷自己
                    if (cArr.get(j).touchBottom(cArr.get(i))) { // 雞之間的下碰撞
            ...
            for (int i = 0; i < rebirthPointArr.size(); i++) { // 啟用復活點
                if (cArr.get(j).touchBottom(rebirthPointArr.get(i))) {
          ...
          if (cArr.get(0).touchBottom(pressBtnArr.get(i))) {
              cArr.get(0).getState().touchBottom(cArr.get(0));
              pressBtnArr.get(i).isPressed(true);
              dArr.get(i).update();
              break;
          }else if(cArr.get(1).touchBottom(pressBtnArr.get(i))){
        	  cArr.get(1).getState().touchBottom(cArr.get(1));
              pressBtnArr.get(i).isPressed(true);
              dArr.get(i).update();
          	break;
            }else {
...
//                      System.out.println("d");
            ...
            if (cArr.get(0).touchRight(pressBtnArr.get(1))||cArr.get(1).touchRight(pressBtnArr.get(1))) { // 在地板走撞到的話直接回覆上一動
                cArr.get(j).getState().touchFloor(cArr.get(j));
                pressBtnArr.get(1).isPressed(true);
                dArr.get(1).update();
                break;
            } else if (cArr.get(0).touchLeft(pressBtnArr.get(1))||cArr.get(1).touchLeft(pressBtnArr.get(1))) { //判斷走路撞到左邊牆壁
                cArr.get(j).getState().touchFloor(cArr.get(j));
                pressBtnArr.get(1).isPressed(true);
                dArr.get(1).update();
                break;
...
//                        System.out.println("l");
    ...
    public void bombUpdate() {
        ...
        for (int n = 0; n < bArr.size(); n++) { // 炸彈的碰撞(
            ...
            for (int i = 0; i < cArr.size(); i++) {  
                if (bArr.get(n).touchBottom(cArr.get(i)) && bArr.get(n).getState() != EXPLODE) {
    ...
    private void mapInitialize() { //地圖載入
        try {
            ArrayList<MapInfo> mapInfo = new MapLoader("scene1.bmp", "scene1.txt").combineInfo();
            ...
            Logger.getLogger(PvPScene1.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void changeScene() {
        ...
        Chicken champion = null;
        for (int i = 0; i < cArr.size(); i++) {
            if (cArr.get(i).getReadyToChange()&& nextSceneDelay.count() || receiveNextScene) {
            	champion = cArr.get(i);
            	if(champion==cArr.get(0)) {
            		results.add(cArr.get(1));
            		results.add(champion); //把冠軍加入名單
            	}else {
            		results.add(cArr.get(0));
            		results.add(champion); 
            	}
            	
            	for (int j = 0; j < results.size(); j++) {
                    results.get(j).setDead(false); // 進入場景前先幫大家復活
                    results.get(j).setState(STILL, State.STILL);
                }
                SceneController.instance().change(new GameOverScenePVP(results)); // 進入結算畫面的場景
                break;
            }
        ...
        return new KeyListener() {
            ...
            public void keyReleased(int commandCode, long trigTime) {
                ...
                if (commandCode == Global.REPLACE) {
                	for(int i=rebirthPointArr.size()-1;i>=0;i--) {
                		if(rebirthPointArr.get(i).canUse()==true) {
                			cArr.get(0).setPosition(rebirthPointArr.get(i).collider().centerX(),rebirthPointArr.get(i).collider().centerY() );
                			break;
                		}
                	}
                }
                ...
                if (commandCode == Global.REPLACE2) {
                	for(int i=rebirthPointArr.size()-1;i>=0;i--) {
                		if(rebirthPointArr.get(i).canUse()==true) {
                			  cArr.get(1).setPosition(rebirthPointArr.get(0).collider().centerX(),rebirthPointArr.get(0).collider().centerY() );
                			break;
                		}
                	}
                }
                ...
                Chicken temp3 = new Chicken(cArr.get(1).collider().centerX(), cArr.get(1).collider().centerY(), cArr.get(1).collider().width(), cArr.get(1).collider().height()); // 利用替身攻擊來判斷移動後會不會撞到
                temp3.translateY(+Global.UNIT_Y +0); // 替身先移動
                ...
                if (commandCode == Global.P1DOWN && cArr.get(0).getState() != DROP && cArr.get(0).getState() != BOOMHEAD && cArr.get(0).getState() != DEAD && !exitPop.isShow()) { // 當按下空白鍵時,後面的判斷是用來避免無限撞頭
                    ...
                    for (int i = pressBtnArr.size() - 1; i >= 0; i--) {
                        if (temp.touchTop(pressBtnArr.get(i))) {
                            ...
                            dArr.get(i).update();
                            break;
                        }else if(!temp.touchTop(pressBtnArr.get(i))&&!temp2.touchTop(pressBtnArr.get(i))){
                      	  pressBtnArr.get(i).isPressed(false);
                      	break;
                        }
                    }
...
//                        arc.play(new Path().sound().ChickenSound().bomb_lay());
                ...
                if (commandCode == Global.P2DOWN && cArr.get(1).getState() != DROP && cArr.get(1).getState() != BOOMHEAD && !exitPop.isShow() && cArr.get(1).getState() != DEAD) { // 當按下空白鍵時,後面的判斷是用來避免無限撞頭
                    ...
                    for (int i = dArr.size() - 1; i >= 0; i--) { 
                        if (temp2.touchTop(dArr.get(i))) {
                            cArr.get(1).setState(BOOMHEAD, ActorAnimator.State.BOOMHEAD);
                            cArr.get(1).painter().setTop(dArr.get(i).collider().bottom() - 1); // 設定為看起來剛好撞到
                            cArr.get(1).painter().setBottom(cArr.get(1).painter().top() + cArr.get(1).collider().height()); 
                            cArr.get(1).collider().setTop(cArr.get(1).painter().top()); // collider 也跟上
                            cArr.get(1).collider().setBottom(cArr.get(1).painter().bottom()); //
                            break;
                    ...
                    for (int i = pressBtnArr.size() - 1; i >= 0; i--) {
                        if (temp2.touchTop(pressBtnArr.get(i))) {
                            cArr.get(1).setState(BOOMHEAD, ActorAnimator.State.BOOMHEAD);
                            cArr.get(1).painter().setTop(pressBtnArr.get(i).collider().bottom() - 1); // 設定為看起來剛好撞到
                            cArr.get(1).painter().setBottom(cArr.get(1).painter().top() + cArr.get(1).collider().height()); //
                            cArr.get(1).collider().setTop(cArr.get(1).painter().top()); // collider 也跟上
                            cArr.get(1).collider().setBottom(cArr.get(1).painter().bottom()); 
                            pressBtnArr.get(i).isPressed(true);
                    ...
                    //transition 的上碰傳送下碰穿越
                    for (int i = transArr.size() - 1; i >= 0; i--) {
                        if (temp2.touchTop(transArr.get(i))) {
                            cArr.get(1).painter().setTop(transArr.get(i).painter().bottom() - 1); // 設定為看起來剛好撞到
                            cArr.get(1).painter().setBottom(cArr.get(1).painter().top() + cArr.get(1).collider().height()); //
                            cArr.get(1).collider().setTop(cArr.get(1).painter().top()); // collider 也跟上
                            cArr.get(1).collider().setBottom(cArr.get(1).painter().bottom()); //
                            cArr.get(1).setState(BOOMHEAD, ActorAnimator.State.BOOMHEAD);
                        	cArr.get(1).translateY(-transArr.get(i).collider().height()-Global.UNIT_Y);
                            break;
                        }
                        if (temp3.touchBottom(transArr.get(i))) {
                        	if(transArr.get(i).getType()== Transition.Type.FLAP ) {
                        		transArr.get(i).setState(PopsAnimator.State.FLAPPING);
                        	}else {
                        		  cArr.get(1).setState(BOOMHEAD, ActorAnimator.State.BOOMHEAD);
                                	cArr.get(1).translateY(transArr.get(i).collider().height()+Global.UNIT_Y*2);
                        	}
                        }
                    }
                    
                    for (int i = gArr.size() - 1; i >= 0; i--) {
                        if (temp2.touchTop(gArr.get(i))) {
                            if (gArr.get(i).getState() == PopsAnimator.State.GEMWALL) {
                                cArr.get(1).setState(BOOMHEAD, ActorAnimator.State.BOOMHEAD);
                                cArr.get(1).painter().setTop(gArr.get(i).collider().bottom() - 1); // 設定為看起來剛好撞到
                                cArr.get(1).painter().setBottom(cArr.get(1).painter().top() + cArr.get(1).collider().height()); //
                                cArr.get(1).collider().setTop(cArr.get(1).painter().top()); // collider 也跟上
                                cArr.get(1).collider().setBottom(cArr.get(1).painter().bottom()); //
                            }
                            if (gArr.get(i).getState() == PopsAnimator.State.GEM) { //變成寶石時可以吃掉
                                gArr.get(i).setState(PopsAnimator.State.GEMGET);
                                cArr.get(1).setGemNumber(cArr.get(1).getGemNumber() + 1);
                            }
                            break;
                        }
                    }
